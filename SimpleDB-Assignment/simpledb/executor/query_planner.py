...
class QueryPlanner:
    ...
    def __init__(self, dbms: DatabaseManager, use_hash_join: bool = True):
        self.dbms = dbms
    ...
    def _build_iterator(self, node: LogicalPlanNode) -> AccessIterator:
        ...
        if node.operation == "access":
            ...
            join_condition = node.kwargs["join_condition"]
            if self.use_hash_join:
                return HashJoin(left_iterator, right_iterator, join_condition)
            return NestedLoopJoin(left_iterator, right_iterator, join_condition)
